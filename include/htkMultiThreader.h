/*=========================================================================
 *=========================================================================*/
#ifndef __htkMultiThreader_h
#define __htkMultiThreader_h
...
public:
  ...
  htkTypeMacro(MultiThreader, Object);
  ...
  htkGetConstMacro(NumberOfThreads, ThreadIdType);

  /** Set/Get the maximum number of threads to use when multithreading.  It
   * will be clamped to the range [ 1, HTK_MAX_THREADS ] because several arrays
   * are already statically allocated using the HTK_MAX_THREADS number.
  ...
  /** Create a new thread for the given function. Return a thread id
     * which is a number between 0 and HTK_MAX_THREADS - 1. This
...
#undef ThreadInfoStruct
  ...
  struct ThreadInfoStruct {
    ...
    enum { SUCCESS, HTK_EXCEPTION, HTK_PROCESS_ABORTED_EXCEPTION, STD_EXCEPTION, UNKNOWN } ThreadExitCode;
  };
...
private:
   ...
   *  (0, 1, 2, .. HTK_MAX_THREADS-1), the thread count, and a pointer
   *  to void so that user data can be passed to each thread. */
  ThreadInfoStruct m_ThreadInfoArray[HTK_MAX_THREADS];

  /** The methods to invoke. */
  ThreadFunctionType m_SingleMethod;
  ThreadFunctionType m_MultipleMethod[HTK_MAX_THREADS];

  /** Storage of MutexFunctions and ints used to control spawned
   *  threads and the spawned thread ids. */
  int                 m_SpawnedThreadActiveFlag[HTK_MAX_THREADS];
  MutexLock::Pointer  m_SpawnedThreadActiveFlagLock[HTK_MAX_THREADS];
  ThreadProcessIDType m_SpawnedThreadProcessID[HTK_MAX_THREADS];
  ThreadInfoStruct    m_SpawnedThreadInfoArray[HTK_MAX_THREADS];

  /** Internal storage of the data. */
  void *m_SingleData;
  void *m_MultipleData[HTK_MAX_THREADS];
   ...
   *  HTK_MAX_THREADS and greater than zero. */
  ...
  static HTK_THREAD_RETURN_TYPE SingleMethodProxy(void *arg);
